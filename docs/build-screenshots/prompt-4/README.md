...
| Desktop nav row uses full category labels (no abbreviations) | `nav-bar.png` navy row: all 10 categories rendered fully within the 1366px viewport: Bedroom Supplies / Buffet & Banquet / F & B Solutions / Food Services / Housekeeping Supplies / Kitchen Services / Kitchen Utensils / Laundry Solutions / Spare Parts / Tableware — no truncation, no horizontal scroll (font-size reduced from `text-[13px]` to `text-[12px]` and padding from `px-3` to `px-2.5` per overflow-fallback chain) |
| Flash Deals red button removed from desktop nav | `nav-bar.png`: rightmost element of navy row is "Tableware ⌄" with chevron, no `bg-destructive` red pill present |
