...
BEGIN
  ...
  IF p_coupon_code IS NOT NULL AND p_coupon_code <> '' THEN
    ...
    IF NOT v_coupon.is_active THEN
      RETURN jsonb_build_object('success', false, 'error', 'This coupon is not currently active');
    END IF;

    -- Validate date range
    IF v_coupon.start_date IS NOT NULL AND v_coupon.start_date > v_now THEN
      RETURN jsonb_build_object('success', false, 'error', 'This coupon is not yet active');
    END IF;
    ...
    IF v_coupon.max_uses IS NOT NULL AND COALESCE(v_coupon.used_count, 0) >= v_coupon.max_uses THEN
      RETURN jsonb_build_object('success', false, 'error', 'This coupon has reached its usage limit');
    END IF;

    -- Validate per-user usage limit
    IF v_coupon.max_uses_per_user IS NOT NULL THEN
      SELECT COUNT(*) INTO v_coupon_user_usage
      FROM coupon_usage
      WHERE coupon_id = v_coupon.id
        AND user_id = auth.uid();

      IF v_coupon_user_usage >= v_coupon.max_uses_per_user THEN
        RETURN jsonb_build_object('success', false, 'error', 'You have already used this coupon');
      END IF;
    END IF;

    -- Validate minimum order amount (against full subtotal)
    IF v_coupon.min_order_amount IS NOT NULL AND v_subtotal < v_coupon.min_order_amount THEN
      RETURN jsonb_build_object('success', false, 'error',
        'Minimum order of MMK ' || v_coupon.min_order_amount::text || ' required for this coupon');
    END IF;

    -- Calculate discount (apply only to full-price items)
    IF v_full_price_subtotal > 0 THEN
      IF v_coupon.type = 'percentage' THEN
        v_discount := v_full_price_subtotal * (v_coupon.discount_value / 100);
        IF v_coupon.max_discount_amount IS NOT NULL THEN
          v_discount := LEAST(v_discount, v_coupon.max_discount_amount);
        END IF;
      ELSIF v_coupon.type = 'fixed_amount' THEN
        v_discount := v_coupon.discount_value;
      END IF;
      -- Cap discount at full-price subtotal
      v_discount := LEAST(v_discount, v_full_price_subtotal);
    END IF;
  END IF;
  -- ===== END COUPON PROCESSING =====
  ...
  -- ===== COUPON POST-ORDER: record usage & increment count =====
  IF v_coupon IS NOT NULL AND v_discount > 0 THEN
    INSERT INTO coupon_usage (coupon_id, user_id, order_id, discount_amount)
    VALUES (v_coupon.id, auth.uid(), v_order_id, v_discount);

    UPDATE coupons SET used_count = COALESCE(used_count, 0) + 1 WHERE id = v_coupon.id;
  END IF;
